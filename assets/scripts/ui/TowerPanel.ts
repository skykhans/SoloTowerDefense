...
export class TowerPanel extends Component {
  ...
  public renderTower(tower: Tower | null): void {
    ...
    if (this.titleLabel) this.titleLabel.string = `${this.resolveTowerName(tower)} Lv.${tower.level}`;
    if (this.infoLabel) {
    ...
    if (this.upgradeValueLabel) {
      this.upgradeValueLabel.string = `升级花费 ${upgradeCost}`;
    }
    ...
    if (this.actionHintLabel) {
      this.actionHintLabel.string =
        currentGold >= upgradeCost ? "金币足够，可以继续升级。" : "金币不足，可先完成当前波次获取更多资源。";
    }
    if (this.upgradeButton) this.upgradeButton.interactable = currentGold >= upgradeCost;
